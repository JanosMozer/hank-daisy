...
- Improved the HankDaisy App MockDevice UI.
...
- [HankDaisy] Removed timer functionality.
...
- [HankDaisy] Improved photo capture flow.
...
- The HankDaisy app streaming UI reflects device availability.
- The HankDaisy app shows errors when incompatible glasses are found.
- The HankDaisy app can now run in background mode, without interrupting streaming (but stopping video decoding).

### Fixed

- Streaming status is set to `stopped` if permission is not granted.
- Fixed UI issues in the HankDaisy app.
...
- HankDaisy app redesigned and updated to the current SDK version.
