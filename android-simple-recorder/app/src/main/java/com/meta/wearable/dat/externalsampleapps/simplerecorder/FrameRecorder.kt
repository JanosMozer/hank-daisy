...
class FrameRecorder(private val context: Context) {
  ...
  fun start(width: Int, height: Int, fps: Int) {
    ...
    outputFile = File(moviesDir, "meta_recording_$name.mp4")
  ...
  @Synchronized
  fun resume() {
    check(codec != null) { "No recording to resume." }
    paused = false
  }

  @Synchronized
  fun pause() {
    check(codec != null) { "No recording to pause." }
    paused = true
  }

  @Synchronized
  fun recordFrame(i420Buffer: ByteBuffer, width: Int, height: Int) {
  ...
  fun finish(): File {
    ...
    releaseCodec()
    return outputFile ?: error("Recording file was not created.")
  }
